...
namespace Lucene.Net.Search.Function
{
	...
	public class CustomScoreQuery:Query, System.ICloneable
	{
        ...
        class AnonymousCustomScoreProvider : CustomScoreProvider
        {
            ...
            public override double CustomScore(int doc, double subQueryScore, double[] valSrcScores)
            {
                return parent.CustomScore(doc, subQueryScore, valSrcScores);
            }

            public override double CustomScore(int doc, double subQueryScore, double valSrcScore)
            {
		...
		public virtual double CustomScore(int doc, double subQueryScore, double[] valSrcScores)
		{
        ...
        [Obsolete("Will be removed in Lucene 3.1")]
		public virtual double CustomScore(int doc, double subQueryScore, double valSrcScore)
		{
		...
		private class CustomScorer:Scorer
		{
			...
			private double[] vScores; // reused in score() to avoid allocating this array for each doc 
			...
			internal CustomScorer(CustomScoreQuery enclosingInstance, Similarity similarity, IndexReader reader, CustomWeight w, Scorer subQueryScorer, Scorer[] valSrcScorers):base(similarity)
			{
				...
				this.reader = reader;
				this.vScores = new double[valSrcScorers.Length];
